...
export function TablesContent({ tablesAnimationPhase }: { tablesAnimationPhase: 'hidden' | 'content-fading-in' | 'visible' | 'content-fading-out' }) {
  ...
  const columnWidths = useMemo(() => {
    const baseWidths = {
      ...
      eclinicalworks: 210, // Updated per request
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      >
        <div style={{ width: `${columnWidths.totalWidth}px` }}>
          {/* Table Header - NOW ANIMATED */}
          <div className={`bg-gray-50 border-b border-gray-200 sticky top-0 z-10 transition-all duration-500 ${
            tablesAnimationPhase === 'content-fading-in' || tablesAnimationPhase === 'visible' ? 'opacity-100' : 'opacity-0'
          }`}>
            <div className="flex" style={{ height: '35px' }}>
              {/* Checkbox column - ANIMATED */}
              <div 
                className={`flex items-center justify-center border-r border-gray-200 shrink-0 transition-all duration-500 ${
                  tablesAnimationPhase === 'content-fading-in' || tablesAnimationPhase === 'visible' ? 'opacity-100' : 'opacity-0'
                }`}
                style={{ 
                  width: `${columnWidths.checkbox}px`,
                  transitionDelay: tablesAnimationPhase === 'content-fading-in' ? '0ms' : '0ms'
                }}
              >
                <input 
                  type="checkbox" 
              ...
              {columns.map((column, index) => {
                ...
                return (
                  <div 
                    ...
                    } bg-gray-50 shrink-0 transition-all duration-500 ${
                      tablesAnimationPhase === 'content-fading-in' || tablesAnimationPhase === 'visible' ? 'opacity-100' : 'opacity-0'
                    }`}
                    style={{ 
                      width: `${width}px`,
                      transitionDelay: tablesAnimationPhase === 'content-fading-in' ? `${(index + 1) * 50}ms` : '0ms'
                    }}
                  >
          ...
          <div className="bg-white">
            {raceMappings.map((mapping, rowIndex) => (
              <div key={mapping.id} className="flex hover:bg-gray-50" style={{ height: '48px' }}>
                {/* Checkbox column */}
                <div 
                  className={`flex items-center justify-center px-3 py-3 border-r border-b border-gray-100 bg-white shrink-0 transition-all duration-500 ${
                    tablesAnimationPhase === 'content-fading-in' || tablesAnimationPhase === 'visible' ? 'opacity-100' : 'opacity-0'
                  }`}
                  style={{ 
                    width: `${columnWidths.checkbox}px`,
                    transitionDelay: tablesAnimationPhase === 'content-fading-in' ? `${(rowIndex * 40) + 400}ms` : '0ms' // Offset by header animation time
                  }}
                >
                ...
                {/* ID column */}
                <div 
                  className={`flex items-center px-3 py-3 border-r border-b border-gray-100 bg-white shrink-0 transition-all duration-500 ${
                    tablesAnimationPhase === 'content-fading-in' || tablesAnimationPhase === 'visible' ? 'opacity-100' : 'opacity-0'
                  }`}
                  style={{ 
                    width: `${columnWidths.id}px`,
                    transitionDelay: tablesAnimationPhase === 'content-fading-in' ? `${(rowIndex * 40) + 450}ms` : '0ms' // Offset by header animation time
                  }}
                >
                ...
                <div 
                  ...
                  style={{ 
                    width: `${columnWidths.standard}px`,
                    transitionDelay: tablesAnimationPhase === 'content-fading-in' ? `${(rowIndex * 40) + 500}ms` : '0ms' // Offset by header animation time
                  }}
                >
                ...
                <div 
                  ...
                  style={{ 
                    width: `${columnWidths.cerner}px`,
                    transitionDelay: tablesAnimationPhase === 'content-fading-in' ? `${(rowIndex * 40) + 550}ms` : '0ms' // Offset by header animation time
                  }}
                >
                  <span className="text-sm text-gray-900 whitespace-nowrap overflow-hidden">
                    {mapping.cerner}
                ...
                <div 
                  ...
                  style={{ 
                    width: `${columnWidths.epic}px`,
                    transitionDelay: tablesAnimationPhase === 'content-fading-in' ? `${(rowIndex * 40) + 600}ms` : '0ms' // Offset by header animation time
                  }}
                >
                  <span className="text-sm text-gray-900 whitespace-nowrap overflow-hidden">
                    {mapping.epic}
                ...
                <div 
                  ...
                  style={{ 
                    width: `${columnWidths.eclinicalworks}px`,
                    transitionDelay: tablesAnimationPhase === 'content-fading-in' ? `${(rowIndex * 40) + 650}ms` : '0ms' // Offset by header animation time
                  }}
                >
                  <span className="text-sm text-gray-900 whitespace-nowrap overflow-hidden">
                ...
                {/* Athena column */}
                <div 
                  className={`flex items-center px-3 py-3 border-b border-gray-100 bg-white shrink-0 transition-all duration-500 ${
                    tablesAnimationPhase === 'content-fading-in' || tablesAnimationPhase === 'visible' ? 'opacity-100' : 'opacity-0'
                  }`}
                  style={{ 
                    width: `${columnWidths.athena}px`,
                    transitionDelay: tablesAnimationPhase === 'content-fading-in' ? `${(rowIndex * 40) + 700}ms` : '0ms' // Offset by header animation time
                  }}
                >
                  <span className="text-sm text-gray-900 whitespace-nowrap overflow-hidden">
                    {mapping.athena}

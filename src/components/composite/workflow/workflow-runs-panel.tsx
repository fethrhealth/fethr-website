...
import { StatusIndicator } from '../../product-ui'
...
export const WorkflowRunsPanel: React.FC<WorkflowRunsPanelProps> = ({ runs }) => {
  const getNodeCountBadge = (nodeCount: number) => (
    <span className="font-medium text-[12px] leading-4 inline-flex items-center text-nowrap rounded-lg border px-[5px] py-px border-[#EEEFF1] bg-[#F4F5F6] text-[#5C5E63] gap-x-[4px] pr-[6px] pl-[3px]">
  ...
  const getRunStatusIcon = (status: string, isCurrentRun: boolean = false) => {
      ...
      {runs.map((run, index) => (
        <div key={run.id} className="border-[#EEEFF1] border-b px-0.5 py-0.5">
          <div className={`flex items-center gap-x-2 rounded-md px-3.5 pt-2.5 pb-[9px] ${
            ...
            {getRunStatusIcon(run.status, run.isCurrentRun)}
        ...
        <div className="mt-1.5 grid grid-cols-2 gap-2">
            ...
            <div className="flex items-end">
              ...
              <div className="mb-[3px] ml-[6px]">
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                  <rect x="1" y="1" width="12" height="12" rx="4" fill="#DDF9E4" />
                  <rect x="1" y="1" width="12" height="12" rx="4" stroke="#C7F4D3" />
                  <path d="M7 4.33268L7 9.66602M7 9.66602L9 7.76125M7 9.66602L5 7.76125" stroke="#0B935D" strokeWidth="0.8" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
            </div>
          ...
          <div className="relative m-[0.5px] flex flex-col rounded-[14px] border border-[#EEEFF1] p-[11px] col-span-2">
            ...
            <div className="mt-4 h-[3px] w-full overflow-hidden rounded-full bg-[#F4F5F6]">
              <div className="h-full rounded-full bg-[#0EB472] transition-[width] duration-300 ease-linear" style={{width: '89.6%'}} />
            </div>
          </div>
        </div>
      </div>

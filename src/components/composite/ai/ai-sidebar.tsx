...
export const AISidebar: React.FC<AISidebarProps> = ({ visible, exiting }) => {
  ...
  useEffect(() => {
    if (visible && !exiting) {
      ...
      // Start typing animation immediately (0-0.5s)
      let currentText = '';
      const textToType = fullText;
      const typingSpeed = 500 / textToType.length; // Complete in 0.5s
      ...
      const typeText = (index: number) => {
        if (index <= textToType.length) {
          ...
          // Simulate "Enter" press and move to chat
          setTimeout(() => {
            // Add a small pause to simulate "Enter" press
            const enterKey = document.createElement('div');
            enterKey.className = 'absolute right-2 bottom-2 bg-blue-600 text-white rounded-full w-8 h-8 flex items-center justify-center transition-all duration-150 transform scale-95';
            enterKey.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m5 12 7-7 7 7" /><path d="M12 19V5" /></svg>`;
            document.querySelector('.chat-input-container')?.appendChild(enterKey);

            // Remove enter key and move text to chat after brief animation
            setTimeout(() => {
              enterKey.remove();
              setTypingText(''); // Clear typing area
              setShowChatMessage(true); // Show in chat

              // Notify parent that chat message is shown (for workflow timing)
              const event = new CustomEvent('chatMessageShown');
